...
def test_get_full_product_urls_from_homepage_uses_best_seller_api(monkeypatch):
    class DummySession:
        ...
        def get(self, url, *args, **kwargs):
            if url == "https://www.trendyol.com/cok-satanlar?type=bestSeller&webGenderId=1":
                class DummyHTMLResponse:
                    status_code = 200
                    text = "<html><body><a href='/not-a-product'>skip</a></body></html>"
                return DummyHTMLResponse()

            if url == "https://apigw.trendyol.com/discovery-sfint-browsing-service/api/top-rankings-v2/top-ranking-contents":
